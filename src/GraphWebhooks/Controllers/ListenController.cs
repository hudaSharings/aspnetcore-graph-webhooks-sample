...
public class ListenController : Controller
{
    ...
    public async Task<IActionResult> Index([FromQuery] string? validationToken = null)
    {
        ...
        foreach (var notification in notifications.Value.Where(n => n.EncryptedContent != null))
        {
            ...
            try
            {
                var message = await notification.EncryptedContent!.DecryptAsync<Message>(async (id, thumbprint) =>
                {
                ...
                if (message != null)
                {
                    ...
                    clientNotifications.Add(new ClientNotification(new
                    {
                        ...
                        Sender = message?.Sender?.EmailAddress?.Name ?? "UNKNOWN",
                        Message = message?.BodyPreview?.ToString() ?? string.Empty,
                        HasAttachments = message?.HasAttachments ?? false
                    }));

                    // Get the Attachment files and Process them if any
    ...
    private async Task GetMessagesAsync(IEnumerable<ChangeNotification> notifications)
    {
        ...
        foreach (var notification in notifications)
        {
            ...
            clientNotifications.Add(new ClientNotification(new
            {
                ...
                Id = message?.Id ?? string.Empty,
                Sender = message?.Sender?.EmailAddress?.Name ?? "UNKNOWN",
                Message = message?.Body?.Content?.ToString() ?? string.Empty,
                HasAttachments = message?.HasAttachments ?? false,
                //AttachmentCount = message?.Attachments?.Count ?? 0,
                //inlineAttachments = message?.Attachments?.Where(a => a.IsInline.HasValue && a.IsInline.Value==true).Count() ?? 0,
    ...
    private async Task ProcessAttachments(Message? message)
    {
        var fileAttachmentResponse = await graphClient.Me.Messages[message.Id].Attachments.GetAsync();
        if (fileAttachmentResponse?.Value != null)
        {
            foreach (FileAttachment item in fileAttachmentResponse.Value)
            {
                if (item?.ContentBytes?.Length > 0)
                {
                    try
                    {
                        // Copy the attachment file to the root directory of the project
                        var attachmentFilePath = Path.Combine(Directory.GetCurrentDirectory(), item.Name);
                        using (var fileStream = new FileStream(attachmentFilePath, FileMode.Create, FileAccess.Write, FileShare.None))
                        {
                            // Create a new file and write the byte array content to it
                            await fileStream.WriteAsync(item.ContentBytes, 0, item.ContentBytes.Length);
                        }
                    }
                    catch (Exception ex)
                    {
