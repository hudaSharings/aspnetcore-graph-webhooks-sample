    ...
    public async Task<IActionResult> AppOnly()
    {
        try
        {
            ...
            var subscription = new Subscription
            {
                ...
                NotificationUrl = $"{notificationHost}/listen",
                LifecycleNotificationUrl = $"{notificationHost}/lifecycle",
                Resource = $"/users/{userId}/messages?$select=id,subject,bodyPreview,hasAttachments,sentDateTime,receivedDateTime,sender,conversationId,conversationIndex",
              //  Resource = "/me/mailfolders('inbox')/messages?$select=id,subject,body,bodyPreview,hasAttachments,sentDateTime,receivedDateTime,sender,toRecipients",
                ClientState = Guid.NewGuid().ToString(),
